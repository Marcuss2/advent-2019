...
impl Direction {
    fn new(s : &str) -> Direction {
        match s.chars().nth(0) {
            Some('L') => Direction::Left(s[1..].parse().unwrap()),
            Some('D') => Direction::Down(s[1..].parse().unwrap()),
            Some('U') => Direction::Up(s[1..].parse().unwrap()),
            Some('R') => Direction::Right(s[1..].parse().unwrap()),
            _ => {
                Direction::Left(0)
            }
        }
